...
class Window_Message < Window_Selectable
  ...
  def initialize
    ...
    @contents_x = 0             # X coordinate for drawing next (0)character
    @contents_y = 0             # Y coordinate for drawing next (0)character
  ...
  def update_show_fast
    if self.pause or self.openness < 255
      @show_fast = false
    elsif Input.trigger?(PadConfig.decision) and @wait_count < 2
      @show_fast = true
    elsif not Input.press?(PadConfig.decision)
      @show_fast = false
    end
