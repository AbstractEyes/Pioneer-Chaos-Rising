...
class Window_Base < Window
  ...
  def draw_sized_icon(resized_rect, icon_index, enabled = true)
    ...
    self.contents.stretch_blt(resized_rect, bitmap_i, rect_i, enabled ? 255 : 100)
  end
  ...
  def draw_character_face(character_name, character_index, x, y, background = true)
    return if character_name == nil
    bitmap = Cache.character(character_name)
    sign = character_name[/^[\!\$\~]./]
    p1 = 1
    ...
    n = character_index
    src_rect = Rect.new((n%4*3+p1)*cw, (n/4*4)*ch+8, cw, 32)
    if background
      self.contents.fill_rect(x - cw / 2, y - ch, cw, 32, PHI.color(:BLACK, 255))
      self.contents.fill_rect(x - cw / 2+2, y - ch+2, cw-4, 32-4, PHI.color(:BLACK, 0))
    end
  ...
  def draw_currency_value(value, x, y, width)
    ...
    self.contents.font.color = normal_color
    self.contents.draw_text(x, y, width-cx-2, WLH, Integer(value), 2)
    self.contents.font.color = system_color
    self.contents.draw_text(x, y, width, WLH, Vocab::gold, 2)
  end

  def draw_element_big(x,y,ele_id)
    bitmap = Cache.element
    rect = Rect.new(ele_id*32,0,64,64)
    self.contents.blt(x, y, bitmap, rect)
    bitmap.dispose
  end

  def currency_icon_index(type=:gold)
    case type
      when :gold, :markers
        return PHI::CURRENCY::ICONS[0]
      when :resource, :collateral
        return PHI::CURRENCY::ICONS[1]
      when :carbon #carbon
        return PHI::CURRENCY::ICONS[2]
      when :metal #metal
        return PHI::CURRENCY::ICONS[3]
      when :elemental #elemental
        return PHI::CURRENCY::ICONS[4]
      when :chaos, :chaotic
        return PHI::CURRENCY::ICONS[5]
    end
  end

  def currency_color(type=:gold)
    case type
      when :gold, :markers
        return PHI.color(PHI::CURRENCY::COLOR[0])
      when :resource, :collateral
        return PHI.color(PHI::CURRENCY::COLOR[1])
      when :carbon #carbon
        return PHI.color(PHI::CURRENCY::COLOR[2])
      when :metal #metal
        return PHI.color(PHI::CURRENCY::COLOR[3])
      when :elemental #elemental
        return PHI.color(PHI::CURRENCY::COLOR[4])
      when :chaotic, :chaos
        return PHI.color(PHI::CURRENCY::COLOR[5])
    end
  end
