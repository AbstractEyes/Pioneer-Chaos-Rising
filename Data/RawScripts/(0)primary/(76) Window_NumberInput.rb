...
class Window_NumberInput < Window_Base
  ...
  def update
    super
    if self.active
      if Input.repeat?(PadConfig.up) or Input.repeat?(PadConfig.down)
        Sound.play_cursor
        place = 10 ** (@digits_max - 1 - @index)
        n = @number / place % 10
        @number -= n * place
        n = (n + 1) % 10 if Input.repeat?(PadConfig.up)
        n = (n + 9) % 10 if Input.repeat?(PadConfig.down)
        @number += n * place
        refresh
      end
      last_index = @index
