...
class Game_Character
  ...
  attr_accessor :shoved                   # shove begun already
  ...
  def collide_with_characters?(x, y)
    for event in $game_map.events_xy(x, y)          # Matches event position
      unless event.through                          # Passage OFF?
        next if $game_temp.in_battle && event.battler != nil
        return true if self.is_a?(Game_Event)       # Self is event and not in battle
        return true if event.priority_type == 1     # Target is normal char
      end
    end
    if @priority_type == 1                          # Self is normal char
      return true if $game_player.pos_nt?(x, y) && !$game_temp.in_battle     # Matches player position
  ...
  def screen_x_adjusted(adjusted_value)
    return ($game_map.adjust_x(@real_x) + 8007) / 8 - 1000 + 16 + adjusted_value
  end
  #--------------------------------------------------------------------------
  ...
  def screen_y_adjusted(adjusted_value)
    y = ($game_map.adjust_y(@real_y) + 8007) / 8 - 1000 + 32 + adjusted_value
    y -= 4 unless object?
    if @jump_count >= @jump_peak
      n = @jump_count - @jump_peak
    else
  ...
  def update_shove
    if shoved?
      if @shove_frames > @max_shove_frames
        @direction_fix = false
        @shoved = false
        straighten
      else
  ...
  #--------------------------------------------------------------------------
  def movable?
    if @battler != nil && $game_temp.in_battle
      return false if is_player? && (!@battler.battle_movement || dead?)
    end
    return false if overwritten_animation? or moving? or shoved?          # Overwritten animation or moving or shoved
    return false if !is_player? && (@battler != nil && @battler.dead?)    # Dead, so cannot move.
