...
class Game_Map
  ...
  def calc_parallax_x(bitmap)
    if bitmap == nil
      ...
      w1 = bitmap.width - $screen_width
      w2 = @map.width * 32 - $screen_width
      if w1 <= 0 or w2 <= 0
        return 0
      else
  ...
  def calc_parallax_y(bitmap)
    if bitmap == nil
      ...
      h1 = bitmap.height - $screen_height
      h2 = @map.height * 32 - $screen_height
      if h1 <= 0 or h2 <= 0
        return 0
      else
  ...
  #--------------------------------------------------------------------------
  def autoplay
    Audio.bgm_play('Audio/BGM/' + @map.bgm.name + '.mp3', $system.music_volume) if @map.autoplay_bgm && !@map.bgm.name.empty?
    Audio.bgs_play('Audio/BGS/' + @map.bgs.name + '.mp3', $system.ambient_volume) if @map.autoplay_bgs && !@map.bgs.name.empty?
  end
