...
# A debugger script.
# Created by Mithran
# Modified by Funplayer(Philip Coelho)
# hosted at rpgmakervx.net; forums.rpgmakerweb.com
# modified version exclusive to Pioneercraft http://www.pioneercraft.net
...
v 1.2
  Altered the way the flow of the main methods function.  This cuts down on clipping
  and lag generated by disposed graphics, allowing for a smoother lag-free update.
...
GRAPHIC_OBJECT_LEAK = false 
# when true, displays a list of undisposed graphical objects
# every time the scene changes.  This includes all graphical objects

GRAPHIC_OBJECT_NOTIFY_CRITICAL = false 
# when true, displays information regarding critical 
# graphical object disposal oversights on scene switch.  These are the errors 
# that could otherwise turn into a Game.exe crash.

# The above two (0)options print a message directly to the screen.

GRAPHIC_OBJECT_DEBUG_FILE = false
# makes a file (gobj.txt) in the game directory containing
...
GRAPHIC_OBJECT_LOG_NON_CRITICAL = false
# if set to true creates log entries for non-critical objects that are not disposed
# between scenes.  Only works if GRAPHIC_OBJECT_DEBUG_FILE is also set to true.
...
GRAPHIC_OBJECT_DEBUG_CRITICAL_ = false
...
# For v1.2 I rewrite the way the data flows, allowing for a smoother
# bitmap to bitmap transition, each checking for its own catch.
[Sprite, Plane, Window, Tilemap].each { |cl|
  class << cl
    ...
    def new(*args)
      obj = new_gobj(*args)
      ary = [obj, $scene.class]
      ary.push(caller) if GRAPHIC_OBJECT_DEBUG_FILE # add caller list if debug file is enabled
  ...
  cl.class_eval {
    ...
    def dispose
      if GRAPHIC_OBJECT_DEBUG_CRITICAL_ && viewport && viewport.disposed?
        o = $gobj.find { |a| a[0] == self }
        print "#{o[0]} created in #{o[1]} is attempting to dispose with a disposed viewport!" if GRAPHIC_OBJECT_NOTIFY_CRITICAL
        if GRAPHIC_OBJECT_DEBUG_FILE && !o[3]
...
class Scene_Base
  ...
  def main
    if !@gobj && $TEST && $gobj.size > 0 
      p 'Live Graphical Object List:', $gobj.collect { |o| o[0..1] } if GRAPHIC_OBJECT_LEAK
      $gobj.clone.each { |o| 
        next o[0].gobj_exempt if o[0].disposed?
        critical = o[0].viewport && o[0].viewport.disposed? 
        print "#{o[0]} created in #{o[1]} is a potential for Game.exe crash!" if GRAPHIC_OBJECT_NOTIFY_CRITICAL && critical
        if GRAPHIC_OBJECT_DEBUG_FILE && !o[3] && (critical or GRAPHIC_OBJECT_LOG_NON_CRITICAL)
          gobj_log_to_file(o, critical)
          o[3] = true # do not log again this instance
        end
        if GRAPHIC_OBJECT_LAZY && critical
          o[0].dispose
        end
      } # close $gobj.each
