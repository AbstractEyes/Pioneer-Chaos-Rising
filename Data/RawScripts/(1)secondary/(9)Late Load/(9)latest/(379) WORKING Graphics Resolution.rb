...
class << Graphics
  ...
  @@redrawWindow     = Win32API.new('user32', 'RedrawWindow', 'l','l')

  @@ratioValue       = 1.333333333333333
  ...
  def initializeMetrics
    ...
    screenWidth = @@getSystemMetrics.call(0)
    screenHeight = @@getSystemMetrics.call(1)
    @@height_leftover = (@@getSystemMetrics.call(6)+7)*2 + @@getSystemMetrics.call(4)
    @@width_leftover = (@@getSystemMetrics.call(5)+8)*2
    @@width_multiplier = defWidth + @@width_leftover
    @@height_multiplier = defHeight + @@height_leftover
  ...
  def changed?
    ...
    if $def_x != rect[0] or $def_y != rect[1] or 
          $def_width != rect[2] - @@width_leftover or $def_height != rect[3] - @@height_leftover
      # For windows 7, -39
      $def_x = rect[0]
      $def_y = rect[1]
      $def_width = rect[2] - @@width_leftover
      $def_height = rect[3] - @@height_leftover
      $system.x = $def_x
      $system.y = $def_y
      $system.w = $def_width
      $system.h = $def_height
  ...
  def force_resolution
    ...
    $def_width = 640 if $def_width < 640
    $def_height = 480 if $def_height < 480
    ...
    $def_width = $def_width - ($def_width % 20) if $def_width % 20 != 0
    $def_height = $def_height - ($def_height % 15) if $def_height % 15 != 0
