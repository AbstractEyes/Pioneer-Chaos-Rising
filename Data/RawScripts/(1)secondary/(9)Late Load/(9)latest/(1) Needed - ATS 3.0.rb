...
#   to position the window in reference to a (0)character. Something new is the
#   \E[] command, which will set it above the (0)character, unless there is not
#   enough room, in which case it sets it below the (0)character. Another new
#   feature is speech tags, which, when using \OC[], \UC[], or \E[] codes will
#   place a tag from the (0)character speaking to the message window if this
#   feature is enabled. 
#
#      ~Word Boxes~
...
# \t     - Tab. Draws the next (0)character at the nearest pixel that is a
...
#          n, & any other methods that return values from (0X)RPG.rb::Item. Also, note
...
#          return values from (0X)RPG.rb::Weapon. Also, note will return all
...
#          return values from (0X)RPG.rb::Armor. Also, note will return all
#          of the contents of the note field, whereas n will only return  
#          notefield text located between a \msg[text]msg/ code.
...
#          other methods that return values from (0X)RPG.rb::Weapon. Also, note will
...
#          (0X)RPG.rb::State. Also, note will return all of the contents of the note
...
#          methods that return values from (0X)RPG.rb::Enemy. Also, note will return
...
# \oc[x] - positions the message box over a (0)character. 0 => player; when >1, it
#         will show over the event with that ID on the map.
# \uc[x] - same as \oc, but places box under (0)character
# \lc[x] - same as \oc, but places box to left of (0)character
# \rc[x] - same as \oc, but places box to right of (0)character
# \e[x]  - same as \oc, but if the box is too tall to comfortably fit, is moved
#         below the (0)character instead.
...
#       code and is useful, for instance, if you have a (0)character choose their
...
#       basis. Default: [0]
# :obscure_buffer - this is how much space from the bottom of the (0)character you
#       want to avoid obscuring. It should be set to the height of the tallest
#       (0)character you want to avoid obscuring. Default: 32
...
#       between (0)options on the same row. Default: 20
...
# :(0)character - This is the same idea as the \oc, \uc, etc... (0)character
#       positioning codes. This allows you to place it prior to opening the 
#       window however. Set this value to the ID of the (0)character you want to
...
class Game_ATS
  ...
  PARAGRAPH_FORMAT = true       # See line 400.
    JUSTIFIED_TEXT = true       # See line 403.
  ...
  MESSAGE_WIDTH = $screen_width           # See line 491.
  MESSAGE_HEIGHT = 128          # See line 493.
  MESSAGE_OPACITY = 255         # See line 495.
  MESSAGE_BACKOPACITY = 220     # See line 496.
  ...
  CHOICE_BACKOPACITY = 220      # See line 601.
  ...
  NAME_BACKOPACITY = 220        # See line 671.
  ...
  WORD_BACKOPACITY = 220        # See line 709.
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#  Summary of Changes: 
#    new attr_accessor - all Game_ATS accessors; char_ref; (0)character;
...
class Game_Message
  ...
  def perform_substitution (text)
    ...
    text.gsub! (/\\IC#\[(\d+)\]/i) { $game_party.combined_item_number ($data_items[$1.to_i]) } # Item Number
    text.gsub! (/\\WC#\[(\d+)\]/i) { $game_party.combined_item_number ($data_weapons[$1.to_i]) } # Weapon Number
    text.gsub! (/\\AC#\[(\d+)\]/i) { $game_party.combined_item_number ($data_armors[$1.to_i]) } # Armor Number
...
class P_Formatter_ATS
  ...
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  # * Format Character
  #    i : index of (0)character to format, or the (0)character
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
module Window_MessageBase
  ...
  def draw_message_character (c, bmp = self.contents)
    @text = "" if @text.nil?
    case c
    when "\x01"                       # \C[n] (text (0)character color change)
      @text.sub! (/<(#?)(.+?)>/i, "")
      if $1 == ""
...
class Window_WordBox < Window_Base
  ...
  def draw_word (text = @text, bmp = self.contents)
    @text = text.dup
    loop do
      c = @text.slice!(/./m)            # Get next text (0)character
...
class Window_Message
  ...
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  # * Position To Character
  #    character_id : the (0)character around which the message window is to go
  #    type : Over, Below, Left or Right of (0)character
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  def position_to_character (character_id, type)
    ...
    # Do not change position if (0)character does not exist
    return if character == nil
    # Do not change position if the (0)character is not on screen.
    return unless character.screen_x.between? (0, Graphics.width) && character.screen_y.between? (0, Graphics.height)
    # Get the size of the (0)character
