...
# Level: Easy, Normal
# 
# The "End Game" option is quite possibly the most useless function in (0X)RPG.rb Maker
...
# menu altogether and giving the player some familiar (0)options seen in many of
# today's commercial (0X)RPG.rb's such as changing window skins, adjust sound volume,
...
module YE
  module SYSTEM
    ...
    # because this is adjustable from the (2)title menu as well.
    TITLE = "System"
    ...
    # determines the window skin used at the (2)title screen. Reference it to
    ...
    # This adjusts the gradient colours for each of the volume (0)controls.
    BGM_COLOUR1 = 30
    BGM_COLOUR2 = 31
    SFX_COLOUR1 = 28
    SFX_COLOUR2 = 29
    ...
    RETURN_TITLE_HELP = "Return back to the (2)title screen."
  ...
  class BGM < AudioFile
    def play
      if @name.empty?
        ...
        #vol = @volume
        #if $system != nil
        #  vol *= ($system.music_volume / 100.0)
        #  vol = [[vol, 0].max, 100].min
        #end
        Audio.bgm_play("Audio/BGM/" + @name, 50, @pitch)
        @@last = self
      end
    end #Play
  end # BGM
  ...
  class ME < AudioFile
    def play
      if @name.empty?
        ...
        #  vol = [[vol, 0].max, 100].min
        #end
        Audio.me_play("Audio/ME/" + @name, 50, @pitch)
      end
    end
  end # ME
  ...
  class SE < AudioFile
    ...
    def play
      unless @name.empty?
        vol = @volume
        #if $system != nil
        #  vol *= ($system.sfx_volume / 100.0)
        #  vol = [[vol, 0].max, 100].min
        #end
        Audio.se_play("Audio/SE/" + @name, 50, @pitch)
      end
    end
    def self.stop
      Audio.se_stop
  ...
  class BGS < AudioFile
    def play
      if @name.empty?
        ...
        vol = @volume
        #if $system != nil
        #  vol *= ($system.ambient_volume / 100.0)
        #  vol = [[vol, 0].max, 100].min
        #end
        Audio.bgs_play("Audio/BGS/" + @name, 50, @pitch)
        @@last = self
      end
    end
  end # BGS
...
class Game_Player < Game_Character
  ...
  def dash?
    if $game_switches[YE::SYSTEM::AUTO_DASH_SWITCH]
      ...
      return false if Input.press?(PadConfig.dash)
      return true
    else
      dash_mso
    end
...
class Scene_End < Scene_Base
  ...
  def start
    ...
    @help_window = Window_Base.new(0, 0, $screen_width, 56)
  ...
  def update
    ...
    if Input.trigger?(PadConfig.cancel)
      ...
      #---
      if Input.repeat?(PadConfig.left) and @system_window.index == 0
      ...
      elsif Input.repeat?(PadConfig.right) and @system_window.index == 0
        $game_variables[YE::SYSTEM::WINDOW_VARIABLE] += 1
        ...
        @help_window.update_windowskin
      #---
      elsif Input.repeat?(PadConfig.left) and @system_window.index == 1
        if Input.press?(PadConfig.dash)
        ...
        RPG::BGM.last.play
        RPG::BGS.last.play
      elsif Input.repeat?(PadConfig.right) and @system_window.index == 1
        if Input.press?(PadConfig.dash)
          $game_variables[YE::SYSTEM::BGM_VOLUME_VAR] -= 10
        else
          $game_variables[YE::SYSTEM::BGM_VOLUME_VAR] -= 1
        ...
        RPG::BGM.last.play
        RPG::BGS.last.play
      elsif Input.trigger?(PadConfig.page_up) and @system_window.index == 1
        ...
        RPG::BGM.last.play
        RPG::BGS.last.play
      elsif Input.trigger?(PadConfig.page_down) and @system_window.index == 1
        if $game_switches[YE::SYSTEM::BGM_MUTE_SWITCH]
          $game_switches[YE::SYSTEM::BGM_MUTE_SWITCH] = false
        else
          $game_switches[YE::SYSTEM::BGM_MUTE_SWITCH] = true
        ...
        RPG::BGM.last.play
        RPG::BGS.last.play
      elsif Input.repeat?(PadConfig.left) and @system_window.index == 2
        if Input.press?(PadConfig.dash)
      ...
      elsif Input.repeat?(PadConfig.right) and @system_window.index == 2
        if Input.press?(PadConfig.dash)
          $game_variables[YE::SYSTEM::SFX_VOLUME_VAR] -= 10
        else
          $game_variables[YE::SYSTEM::SFX_VOLUME_VAR] -= 1
        ...
        Sound.play_cursor
        @sfx_vol_window.refresh
      elsif Input.trigger?(PadConfig.page_up) and @system_window.index == 2
        ...
        Sound.play_decision
        @sfx_vol_window.refresh
      elsif Input.trigger?(PadConfig.page_down) and @system_window.index == 2
        if $game_switches[YE::SYSTEM::SFX_MUTE_SWITCH]
          $game_switches[YE::SYSTEM::SFX_MUTE_SWITCH] = false
        else
          $game_switches[YE::SYSTEM::SFX_MUTE_SWITCH] = true
        ...
        Sound.play_decision
        @sfx_vol_window.refresh
      elsif Input.repeat?(PadConfig.left) and @system_window.index == 3
        if Input.press?(PadConfig.dash)
        ...
        RPG::BGM.last.play
        RPG::BGS.last.play
      elsif Input.repeat?(PadConfig.right) and @system_window.index == 3
        if Input.press?(PadConfig.dash)
          $game_variables[YE::SYSTEM::BGS_VOLUME_VAR] -= 10
        else
          $game_variables[YE::SYSTEM::BGS_VOLUME_VAR] -= 1
        ...
        RPG::BGM.last.play
        RPG::BGS.last.play
      elsif Input.trigger?(PadConfig.page_up) and @system_window.index == 3
        ...
        RPG::BGM.last.play
        RPG::BGS.last.play
      elsif Input.trigger?(PadConfig.page_down) and @system_window.index == 3
        if $game_switches[YE::SYSTEM::BGS_MUTE_SWITCH]
          $game_switches[YE::SYSTEM::BGS_MUTE_SWITCH] = false
        else
          $game_switches[YE::SYSTEM::BGS_MUTE_SWITCH] = true
        ...
        RPG::BGM.last.play
        RPG::BGS.last.play
      elsif Input.trigger?(PadConfig.left) and @system_window.index == 4
        $game_switches[YE::SYSTEM::ANI_SWITCH] = false
        Sound.play_decision
        @system_window.refresh
      elsif Input.trigger?(PadConfig.right) and @system_window.index == 4
        $game_switches[YE::SYSTEM::ANI_SWITCH] = true
        Sound.play_decision
        @system_window.refresh
      elsif Input.trigger?(PadConfig.left) and @system_window.index == 5
        $game_switches[YE::SYSTEM::AUTO_DASH_SWITCH] = true
        Sound.play_decision
        @system_window.refresh
      elsif Input.trigger?(PadConfig.right) and @system_window.index == 5
        $game_switches[YE::SYSTEM::AUTO_DASH_SWITCH] = false
        Sound.play_decision
        @system_window.refresh
      elsif Input.trigger?(PadConfig.left) and @system_window.index == 6
        $game_switches[YE::SYSTEM::INSTANT_TEXT_SWITCH] = true
        Sound.play_decision
        @system_window.refresh
      elsif Input.trigger?(PadConfig.right) and @system_window.index == 6
        $game_switches[YE::SYSTEM::INSTANT_TEXT_SWITCH] = false
        Sound.play_decision
        @system_window.refresh
      elsif Input.trigger?(PadConfig.decision) and @system_window.index == 7
        Sound.play_decision
        RPG::BGM.fade(800)
        RPG::BGS.fade(800)
        RPG::ME.fade(800)
        ...
        Graphics.fadeout(60)
      elsif Input.trigger?(PadConfig.decision) and @system_window.index == 8
        Sound.play_cancel
        return_scene
      end
      #---
