...
################################################################################
#  Unlock Window:
#    If you're tired of that pesky window bugging you, drag and drop this baby.
#    It'll start right up, and you'll be ready to go.
#    
#    If anyone feels like optimizing it, go for it.
#    
#  Made by: Funplayer
#    Use it, modify it, give it away, sell it, I don't care.  
#    Just put my name in the credits, and drop me a 
#    message and I'll buy your game when its on steam.
################################################################################

################################################################################
# Window Styles:
#   Simple functionality objects in hex to communicate with the win32 wrapper.
#   Each object type is predetermined, you can modify the overall structure
#   of front window, by applying different window style variables to it.
# 
# For a full guide of different styles, please consult the Microsoft VC 
# distribution website for further information on each topic.
################################################################################
module Window_Styles
  ...
  # Back window has currently become the main window,
  # the old design was catching the frames from the main RGSS window (backWindow),
  # and passing them to a second window(frontWindow).  I need more experience
  # with win32 to make that work though.
  ...
  # This is a simplified original main window.  The old window
  # was replaced, but this code remains.
  def self.frontWindow
    return (WS_BORDER |
#~         WS_CAPTION   |
        WS_CLIPSIBLINGS   |
#~         WS_DLGFRAME   |
...
class << Graphics  
  ...
  def update
    release_alt if Input.trigger?([18,164,165])
    update_replace
    if changed?
      force_resolution
      initialize_data
  ...
  def initializeMetrics
    ...
    screenWidth = @@getSystemMetrics.call(0)
    screenHeight = @@getSystemMetrics.call(1)
    @@width_multiplier = defWidth + (@@getSystemMetrics.call(5)+6)*2
  ...
  def different_metrics?(rect)
    return (  $def_x != rect[0] or 
              $def_y != rect[1] or 
  ...
  def force_resolution
    ...
    $def_x = 0 if $def_x < 0
    $def_x = (@@screenWidth / 2.0).to_i if $def_x > @@screenWidth
    $def_y = 0 if $def_y < 0
    $def_y = (@@screenHeight / 2.0).to_i if $def_y > @@screenHeight
